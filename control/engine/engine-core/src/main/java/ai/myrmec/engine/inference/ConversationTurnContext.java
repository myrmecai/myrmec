...
/**
 * Engine-internal conversation-turn assembly context (§6.2).
 *
 * <p>Carries everything the input assembler needs to produce one assistant
 * turn for a conversational session: the system prompt (per-conversation
 * override or profile fallback), per-conversation pinned facts, a sliding
 * window of prior messages, and the just-arrived user message. The
 * {@code assistantSequenceNo} is pre-allocated by the engine so the turn's
 * execution frames carry a stable, viewable sequence number from the very
 * first chunk. The assembled transcript ships as the §8.1
 * {@code execution.start} input block; this type never crosses the wire.</p>
 */
@Data
@Builder
@NoArgsConstructor
...
public class ConversationTurnContext {
    ...
    /**
     * Effective system prompt â€” per-conversation override if set,
    ...
    /**
     * Sliding context window â€” most recent N messages, oldest first.
    ...
    /**
     * The user message that triggered this turn â€” duplicated from
    ...
    /** Model handle + decrypted API key (§6.1 model config). */
    private SessionOpenPayload.ModelConfig model;
     ...
     * field â€” it produces a turn identically either way; the distinction is
    ...
    public static class AttachmentDescriptor {
         ...
         * vision â€” the agent should send it as a native image part.
         */
         ...
         * budget past {@code attachment_inline_ratio_max Ã— contextBudget}
