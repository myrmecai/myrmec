 ...
 * (GET /admin/agents/{id}/workers) added with the Agent Host model: a host's
 ...
 */
class AgentAdminControllerWorkersTest extends IntegrationTestBase {
    ...
    void listsTheWorkerReplicasOfAHostWithTheirFsmStatus() {
        ...
        ResponseEntity<List<AgentWorkerResponse>> resp = restTemplate.exchange(
                "/api/v1/admin/agents/" + host.getId() + "/workers",
                HttpMethod.GET,
    ...
    void unknownHostReturns404() {
        ResponseEntity<String> resp = restTemplate.exchange(
                "/api/v1/admin/agents/" + UUID.randomUUID() + "/workers",
                HttpMethod.GET,
                new HttpEntity<>(adminHeaders()),
                String.class);
