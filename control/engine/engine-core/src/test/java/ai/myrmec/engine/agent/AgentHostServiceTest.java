...
class AgentHostServiceTest {
    ...
    void recordHostAnnounceUpdatesControlNodeAndHostTelemetry() {
        ...
        when(agentHostRepository.findById(hostId)).thenReturn(Optional.of(host));
        when(agentHostRepository.save(any(AgentHost.class))).thenAnswer(invocation -> invocation.getArgument(0));
        ...
        verify(agentHostRepository).save(saved.capture());
